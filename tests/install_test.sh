...
for script in "$TMP/root/data/unifi-jpix-tunnel-repair/scripts"/*.sh; do
  test_start "installed script is mode 0755: ${script##*/}"
  assert_eq "$(stat -c %a "$script")" 755
done
for unit in "$TMP/root/etc/systemd/system"/unifi-jpix-tunnel-repair-*; do
...
chmod 777 "$TMP/root/data/unifi-jpix-tunnel-repair/scripts"
printf 'must-remain-unchanged\n' >"$TMP/root/data/unifi-jpix-tunnel-repair/scripts/hostile-marker"
set +e
...
assert_eq "$(cat "$TMP/root/data/unifi-jpix-tunnel-repair/scripts/hostile-marker")" must-remain-unchanged
