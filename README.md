...
詳細な完了・未完了一覧は[Validation](docs/validation.md#現在の実機検証範囲)を正本とします。残Issueが完了するまで、現方式はUniFi管理トンネルを共有するexperimental repairです。
